    ...
    for sink in sinks:
        ...
        if len(preds) == 1:
            ...
            if G.out_degree(pred) == 1:
                ...
                print >> sys.stderr, "contract sink", pred, sink
...
def collapse_chain(G, chain, mermap, path_d):
    ...
    print >> sys.stderr, "shortening {0} -> {1} weight avg: {2}".format(n0, nlast, avg_weights_in_path)
...
def reachability_helper(G, cur, chain, visited, mermap, path_d):
    ...
    if cur in visited:
        if len(chain) >= 2:
            print >> sys.stderr, "chain found!", chain + [cur]
            collapse_chain(G, chain+[cur], mermap, path_d)
        print >> sys.stderr, "already visited", cur
        return
    visited[cur] = 1
...
def find_dangling_sinks(G, path_d, mermap):
    ...
    for sink in cand_sinks:
        ...
        if G.out_degree(pred) == 2: # must be pred -> sink and pred -> n
            a, b = G.successors(pred)
            n = a if b == sink else b
            print "tugging dangling sink", pred, sink, n
            #pdb.set_trace()
            prefix = mermap[pred][:KMER_SIZE-1]
            suffix = mermap[pred][KMER_SIZE-1:]
            extra = mermap[sink][KMER_SIZE-1:]
            # update path_d
            for k in path_d:
                if sink in path_d[k]:
                    assert path_d[k][-1] == sink
                    path_d[k] = path_d[k][:-1]
            # update mermap and remove sink from G
            mermap[pred] = mermap[pred] + extra
            mermap[n] = extra[-(KMER_SIZE-1):] + mermap[n][KMER_SIZE-1:]
            del mermap[sink]
            G.remove_node(sink)
...
def find_minimal_path_needed_to_explain_pathd(G, path_d, keys):
    ...
    for src in sources:
        for sink in sinks:
            ...
            print >> sys.stderr, "number of paths now:", len(paths)
    ...
    for j,k in enumerate(keys):
        if j not in good_for:
            print >> sys.stderr, "missing good_for for", k
